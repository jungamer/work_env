...
RUN yum install -y wget git vim zsh ctags python python-dev gcc g++ readline-devel sudo ncurses-devel subversion cmake openssh-server.x86_64
RUN curl -R -O http://www.lua.org/ftp/lua-5.3.4.tar.gz && \
    ...
    curl https://beyondgrep.com/ack-2.18-single-file > /bin/ack && chmod 0755 /bin/ack && sudo rm -rf /lua-5.3.4* /root/*
